...
# This compares the C++ engine's bot against a Python InferenceNet using the SAME
# hand-rolled net, so it only applies to the torch-free build. On the LibTorch
# build the engine uses torch while InferenceNet is hand-rolled; their agreement
# is already covered (within tolerance) by tests/test_infer_parity.py.
SKIP_TORCH = bool(getattr(nardi, "USES_TORCH", False))
...
if __name__ == "__main__":
    if SKIP_TORCH:
        print("BOT PARITY skipped (LibTorch build; engine<->hand-rolled agreement "
              "is covered by test_infer_parity)")
        sys.exit(0)
    ge, gt = check_greedy()
